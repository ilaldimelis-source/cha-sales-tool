...
var planReferenceData = {
  ...
  'goodhealth': {name:'GoodHealth 1-5',network:'First Health Network',underwriter:'Merchants Benefit Administrators (MBA)',association:'Good Health Distribution Partners',providerSearch:'providerlocator.firsthealth.com',type:'MEC'},
  'harmonycare': {name:'Harmony Care / Sigma Care',network:'First Health Network',underwriter:'American Financial Security Life (AFSLIC)',association:'National Congress of Employers (NCE)',providerSearch:'providerlocator.firsthealth.com',type:'Indemnity'},
  'medfirst': {name:'MedFirst / TrueHealth 1-5',network:'First Health Network',underwriter:'Merchants Benefit Administrators (MBA)',association:'VP Limited Partnership / TVP',providerSearch:'providerlocator.firsthealth.com',type:'MEC'},
  ...
  'access-health': {name:'Access Health Traditional STM',network:'PHCS',underwriter:'AFSLIC',association:'NCE',providerSearch:'providersearch.multiplan.com',type:'STM'},
  'smart-health': {name:'Smart Health STM',network:'PHCS',underwriter:'SLACIC',association:'NCE',providerSearch:'providersearch.multiplan.com',type:'STM'},
  ...
  'galena': {name:'AFRP Galena STM Elite',network:'MultiPlan',underwriter:'SGIC',association:'Association for Responsible Planners',providerSearch:'providersearch.multiplan.com',type:'STM'},
  ...
  'healthchoice': {name:'HealthChoice Silver',network:'MultiPlan',underwriter:'AFSLIC',association:'NCE',providerSearch:'providersearch.multiplan.com',type:'Indemnity'},
...
function initAcademy(){var agentId=localStorage.getItem('visibleAgentId')||'default';var saved=localStorage.getItem('academyProgress_'+agentId);if(saved){academyProgress=JSON.parse(saved);}else{academyProgress={};for(var i=1;i<=5;i++){academyProgress['day'+i]={lessons:[false,false,false,false],complete:false};}}renderDayCards();updateAcademyUI();initBenefitsPanel();}
...
function generateLessonCard(day,lessonIndex,title,desc,time,isComplete,isCurrent){var status=isComplete?'completed':(isCurrent?'current':'locked');var checkContent=isComplete?svgIcons.check:(lessonIndex+1);var tag=isComplete?'Complete':(isCurrent?'Up Next':'Lesson '+(lessonIndex+1));var html='<div class="lesson-card '+status+'">';html+='<div class="lesson-check">'+checkContent+'</div>';html+='<div class="lesson-info"><div class="lesson-tag">'+tag+'</div><h4>'+title+'</h4><p>'+desc+'</p></div>';html+='<span class="lesson-time">'+time+'</span>';if(isCurrent)html+='<button class="lesson-action">Start</button>';html+='</div>';return html;}
function completeLesson(dayNum,lessonNum){academyProgress['day'+dayNum].lessons[lessonNum]=true;var allComplete=academyProgress['day'+dayNum].lessons.every(function(l){return l;});if(allComplete){academyProgress['day'+dayNum].complete=true;showConfetti();}saveAcademyProgress();updateAcademyUI();loadDayContent(dayNum);}
...
function generateDay2Content(dayData){var lessons=dayData.lessons;var currentLesson=lessons.findIndex(function(l){return !l;});if(currentLesson===-1)currentLesson=4;var html='<button class="back-btn" onclick="hideAllDayContent()">'+svgIcons.x+' Back to all days</button>';html+='<div class="day-header"><div class="day-header-icon">'+svgIcons.target+'</div><div><h2>Day 2: Pre-Qualification</h2><p>Master the opening - 4 lessons with real scripts</p></div></div>';html+='<div class="lessons-list">';html+=generateLessonCard(2,0,'Opening Script','Word-for-word when they pick up','4 min',lessons[0],currentLesson===0);html+=generateLessonCard(2,1,'Prequal Questions','Info you need before picking a plan','5 min',lessons[1],currentLesson===1);html+=generateLessonCard(2,2,'STM Knockout Questions','15 conditions for STM eligibility','3 min',lessons[2],currentLesson===2);html+=generateLessonCard(2,3,'Tonality Guide','Sound confident and natural','4 min',lessons[3],currentLesson===3);html+='</div>';if(currentLesson===0)html+=generateLesson2_1();else if(currentLesson===1)html+=generateLesson2_2();else if(currentLesson===2)html+=generateLesson2_3();else if(currentLesson===3)html+=generateLesson2_4();else html+='<div class="pro-tip" style="margin-top:24px;">'+svgIcons.check+'<div class="pro-tip-content"><div class="tip-label">Day 2 Complete!</div><p>You know the opening! Day 3 unlocked.</p></div></div>';return html;}
...
function generateDay3Content(dayData){var lessons=dayData.lessons;var currentLesson=lessons.findIndex(function(l){return !l;});if(currentLesson===-1)currentLesson=4;var html='<button class="back-btn" onclick="hideAllDayContent()">'+svgIcons.x+' Back to all days</button>';html+='<div class="day-header"><div class="day-header-icon">'+svgIcons.chat+'</div><div><h2>Day 3: The Pitch</h2><p>Present plans with confidence</p></div></div>';html+='<div class="lessons-list">';html+=generateLessonCard(3,0,'Return from Hold','Coming back with the SIZZLE','3 min',lessons[0],currentLesson===0);html+=generateLessonCard(3,1,'Price Presentation','Present the monthly cost','4 min',lessons[1],currentLesson===1);html+=generateLessonCard(3,2,'Benefits Script','Walk through coverage','5 min',lessons[2],currentLesson===2);html+=generateLessonCard(3,3,'Plan Lookup','Find network and underwriter','3 min',lessons[3],currentLesson===3);html+='</div>';if(currentLesson===0)html+=generateLesson3_1();else if(currentLesson===1)html+=generateLesson3_2();else if(currentLesson===2)html+=generateLesson3_3();else if(currentLesson===3)html+=generateLesson3_4();else html+='<div class="pro-tip" style="margin-top:24px;">'+svgIcons.check+'<div class="pro-tip-content"><div class="tip-label">Day 3 Complete!</div><p>You can pitch! Day 4 unlocked.</p></div></div>';return html;}
...
function generateLesson3_4(){var html='<div class="lesson-detail active"><div class="lesson-detail-header"><h3>Plan Lookup</h3></div><div class="lesson-detail-body">';html+='<div class="intro-bubble">'+svgIcons.search+'<p>Use this to find network, underwriter, and provider search URL for any plan.</p></div>';html+='<div class="plan-selector"><label>Select a Plan:</label><select id="planSelect" onchange="showPlanInfo()"><option value="">-- Choose a plan --</option>';html+='<optgroup label="First Health Network"><option value="smartchoice">SmartChoice</option><option value="tdk">TDK 1-5</option><option value="goodhealth">GoodHealth 1-5</option><option value="harmonycare">Harmony Care / Sigma Care</option><option value="medfirst">MedFirst / TrueHealth 1-5</option></optgroup>';html+='<optgroup label="PHCS Network"><option value="pinnacle-stm">Pinnacle STM Traditional</option><option value="pinnacle-protect">Pinnacle Protect 1-4</option><option value="access-health">Access Health STM</option><option value="smart-health">Smart Health STM</option><option value="bwa-americare">BWA Americare 2-4</option></optgroup>';html+='<optgroup label="MultiPlan Network"><option value="galena">AFRP Galena STM Elite</option><option value="everest">Everest Summit</option><option value="healthchoice">HealthChoice Silver</option></optgroup></select></div>';html+='<div id="planInfoDisplay"></div>';html+='<div class="pro-tip">'+svgIcons.info+'<div class="pro-tip-content"><div class="tip-label">Pro Tip</div><p>Have this open during calls! You need the network and provider URL for post-close.</p></div></div>';html+='<button class="mark-complete-btn" onclick="completeLesson(3,3)">'+svgIcons.check+' Got it! Mark Complete</button></div></div>';return html;}
function showPlanInfo(){var select=document.getElementById('planSelect');var display=document.getElementById('planInfoDisplay');var planKey=select.value;if(!planKey||!planReferenceData[planKey]){display.innerHTML='';return;}var plan=planReferenceData[planKey];var html='<div class="plan-info-card"><h4>'+plan.name+'</h4><div class="plan-info-grid">';html+='<div class="plan-info-item"><span>Network</span><strong>'+plan.network+'</strong></div>';html+='<div class="plan-info-item"><span>Underwriter</span><strong>'+plan.underwriter+'</strong></div>';html+='<div class="plan-info-item"><span>Association</span><strong>'+plan.association+'</strong></div>';html+='<div class="plan-info-item"><span>Type</span><strong>'+plan.type+'</strong></div>';if(plan.providerSearch)html+='<div class="plan-info-item"><span>Provider Search</span><strong>'+plan.providerSearch+'</strong></div>';html+='</div></div>';display.innerHTML=html;}
function generateDay4Content(dayData){var lessons=dayData.lessons;var currentLesson=lessons.findIndex(function(l){return !l;});if(currentLesson===-1)currentLesson=4;var html='<button class="back-btn" onclick="hideAllDayContent()">'+svgIcons.x+' Back to all days</button>';html+='<div class="day-header"><div class="day-header-icon">'+svgIcons.checkCircle+'</div><div><h2>Day 4: Close & Verify</h2><p>Seal the deal - 4 lessons</p></div></div>';html+='<div class="lessons-list">';html+=generateLessonCard(4,0,'Collect Info','Email, address, phone confirmation','3 min',lessons[0],currentLesson===0);html+=generateLessonCard(4,1,'Payment Collection','Get the card - then STOP TALKING','3 min',lessons[1],currentLesson===1);html+=generateLessonCard(4,2,'Verification Walkthrough','Walk them through DocuSign','4 min',lessons[2],currentLesson===2);html+=generateLessonCard(4,3,'Post-Close Script','What to say after they sign','5 min',lessons[3],currentLesson===3);html+='</div>';if(currentLesson===0)html+=generateLesson4_1();else if(currentLesson===1)html+=generateLesson4_2();else if(currentLesson===2)html+=generateLesson4_3();else if(currentLesson===3)html+=generateLesson4_4();else html+='<div class="pro-tip" style="margin-top:24px;">'+svgIcons.check+'<div class="pro-tip-content"><div class="tip-label">Day 4 Complete!</div><p>You know how to close! Final day unlocked.</p></div></div>';return html;}
function generateLesson4_1(){var html='<div class="lesson-detail active"><div class="lesson-detail-header"><h3>Collect Info</h3></div><div class="lesson-detail-body">';html+='<div class="script-block"><div class="script-header"><span class="script-label">'+svgIcons.doc+' Info Collection</span><button class="copy-btn" data-copy="infoScript" onclick="copyScript(\'infoScript\')">'+svgIcons.copy+' Copy</button></div>';html+='<div class="script-body" id="infoScript"><p>OK Perfect, what is a good email we can send all this over to?</p><p>Would you like your cards issued with a middle initial, or just first and last name?</p><p>What is the physical mailing address for hard-copy documents?</p><p>Is this phone number <span class="fill-in">________</span> the best number for the insurance company to reach you?</p><p>Before we complete enrollment, I need to verify a few items on this recorded line. Can you confirm you are a US citizen or legal resident, and all health information provided is accurate?</p><p>Lastly, for identity verification, please confirm your Social Security number?</p><div class="script-callout wait">'+svgIcons.play+' Wait for answer</div></div></div>';html+='<button class="mark-complete-btn" onclick="completeLesson(4,0)">'+svgIcons.check+' Got it! Mark Complete</button></div></div>';return html;}
...
function generateLesson5_4(){var html='<div class="lesson-detail active"><div class="lesson-detail-header"><h3>Mock Call Checklist</h3></div><div class="lesson-detail-body">';html+='<div class="intro-bubble">'+svgIcons.doc+'<p>Before going live, practice and check off each item:</p></div>';html+='<div class="checklist-items">';var items=['I can say the opening script without reading it','I know all the prequal questions to ask','I can list the 15 STM knockout conditions','I understand the 3 tonality types','I can present price confidently','I know the benefits script flow','I can handle "I need to think about it"','I know to STOP TALKING after asking for payment','I can walk someone through verification','I know the post-close script and CS number (855-736-1590)'];for(var i=0;i<items.length;i++){html+='<div class="checklist-item" onclick="this.classList.toggle(\'checked\');this.querySelector(\'input\').checked=this.classList.contains(\'checked\')"><input type="checkbox"><label>'+items[i]+'</label></div>';}html+='</div>';html+='<div class="pro-tip">'+svgIcons.rocket+'<div class="pro-tip-content"><div class="tip-label">You Are Ready!</div><p>Once all boxes checked, you are ready for your first live call. Read scripts word-for-word until natural!</p></div></div>';html+='<button class="mark-complete-btn" onclick="completeLesson(5,3)">'+svgIcons.check+' Complete Academy!</button></div></div>';return html;}
