...
function setTrainingSearchQuery(v) {
  TRAINING_SEARCH_QUERY = String(v || '')
    .trim()
    .toLowerCase();
  var lib = document.getElementById('page-traininghome');
...
function _caApplySearch() {
  ...
  for (var hi = 0; hi < hits.length; hi++)
    hits[hi].classList.remove('ca-search-hit');
  if (!q) return;
...
function renderTrainingHome() {
  ...
  html +=
    ...
    _caCopyBtn(
      'MEC: ' + planMec + ' STM: ' + planStm + ' Indemnity: ' + planInd
    ) +
    '</div></div></details>';
  ...
  var stages2 = [
    {
      n: 1,
      t: 'Opening',
      b: "Use breaking rapport tonality (down tone). Sound happy. Slow steady pace. Assume it's the customer.",
    ...
    },
    {
      n: 2,
      t: 'Prequalify',
      b: 'Conversational 50/50. Gather eligibility info + listen for buying points + medical info',
    ...
    },
    {
      n: 3,
      t: 'Tie-Down Price',
      b: 'Get their budget BEFORE you pitch. Ask income questions like marketplace does.',
    ...
    },
    {
      n: 5,
      t: 'Teaser',
      b: "Come back with higher energy. Steady pace so they can't interrupt.",
  ...
  var never = [
    ...
    {
      show: '\u274c "No pre-existing issues"',
      copy: '"No pre-existing issues"'
    },
    {
  ...
  var sayInstead = [
    {
      show: '\u2705 "This is a limited benefit plan"',
      copy: '"This is a limited benefit plan"'
    },
    {
      show: '\u2705 "Designed for healthy people"',
      copy: '"Designed for healthy people"'
    },
    {
  ...
  html +=
    '<div class="ca-compliance-hd">MUST DISCLOSE</div><ul class="ca-compliance-list">';
  ...
  html +=
    '<div class="ca-compliance-hd">NEVER SAY</div><ul class="ca-compliance-list">';
  ...
  html +=
    '<div class="ca-compliance-hd">SAY INSTEAD</div><ul class="ca-compliance-list">';
...
function _trnBottomStrip(activeId) {
  ...
  allItems.forEach(function (item) {
    ...
    var border = isActive
      ? '1.5px solid #5B8DEF'
      : '1.5px solid var(--cha-border-default)';
    html +=
...
var svgIcons = {
  ...
  rocket:
    '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4.5 16.5c-1.5 1.26-2 5-2 5s3.74-.5 5-2c.71-.84.7-2.13-.09-2.91a2.18 2.18 0 00-2.91-.09z"/><path d="M12 15l-3-3a22 22 0 012-3.95A12.88 12.88 0 0122 2c0 2.72-.78 7.5-6 11a22.35 22.35 0 01-4 2z"/></svg>',
...
var planReferenceData = {
  ...
  harmonycare: {
    name: 'Harmony Care / Sigma Care',
    network: 'First Health Network',
    underwriter: 'American Financial Security Life Insurance Company (AFSLIC)',
  ...
  'pinnacle-stm': {
    ...
    network: 'PHCS',
    underwriter: 'Everest Reinsurance Company',
  ...
  'access-health': {
    ...
    network: 'PHCS',
    underwriter: 'AFSLIC (American Financial Security Life Insurance Company)',
    association: 'NCE (National Congress of Employers)',
    providerSearch: 'providersearch.multiplan.com',
    type: 'STM'
  },
  'smart-health': {
    ...
    network: 'PHCS',
    underwriter: 'SLACIC (Standard Life and Casualty Insurance Company)',
    association: 'NCE (National Congress of Employers)',
    providerSearch: 'providersearch.multiplan.com',
    type: 'STM'
  },
  ...
  everest: {
    ...
    network: 'MultiPlan',
    underwriter: 'Everest Reinsurance Company',
    association: 'National Congress of Employers',
    providerSearch: 'providersearch.multiplan.com',
    type: 'Indemnity'
  },
  healthchoice: {
...
function renderDayCards() {
  ...
  for (var i = 0; i < dayConfig.length; i++) {
    ...
    html +=
      ...
      '" onclick="selectDay(' +
      d.num +
      ')"><div class="day-icon-circle blue" id="dayIcon' +
      d.num +
      '">' +
      d.num +
      '</div><h3>' +
      d.title +
      '</h3><p class="day-desc">Day ' +
      d.num +
      '</p><div class="day-progress"><div class="day-progress-bar" id="day' +
      d.num +
      'Progress"></div></div><span class="day-status" id="day' +
      d.num +
...
function updateAcademyUI() {
  ...
  for (var d = 1; d <= 5; d++) {
    var dayData = academyProgress['day' + d];
    var dayCompleted = dayData.lessons.filter(function (l) {
      return l;
    }).length;
    completedLessons += dayCompleted;
    var dayPercent = (dayCompleted / 4) * 100;
    var progressBar = document.getElementById('day' + d + 'Progress');
    var statusEl = document.getElementById('day' + d + 'Status');
    var dayCard = document.querySelector('.day-card[data-day="' + d + '"]');
    var iconEl = document.getElementById('dayIcon' + d);
    if (progressBar) progressBar.style.width = dayPercent + '%';
    if (dayData.complete) {
      ...
      if (iconEl) {
        iconEl.classList.remove('blue', 'gray');
        iconEl.classList.add('green');
        iconEl.innerHTML = svgIcons.check;
      }
    } else {
      if (statusEl)
        statusEl.textContent = dayPercent > 0 ? 'In progress' : 'Not started';
      if (dayCard) dayCard.classList.remove('locked');
      if (iconEl) {
        iconEl.classList.remove('gray', 'green');
        iconEl.classList.add('blue');
        iconEl.innerHTML = d;
      }
    }
  }
  var overallPercent = Math.round((completedLessons / totalLessons) * 100);
  var percentEl = document.getElementById('academyProgressPercent');
  ...
  if (ringEl) {
    ...
    var offset = circumference - (overallPercent / 100) * circumference;
...
function selectDay(dayNum) {
  ...
  document
    .querySelector('.day-card[data-day="' + dayNum + '"]')
...
function loadDayContent(dayNum) {
  var container = document.getElementById('day' + dayNum + 'Content');
  if (!container) return;
  var dayData = academyProgress['day' + dayNum];
  if (dayNum === 1) container.innerHTML = generateDay1Content(dayData);
  else if (dayNum === 2) container.innerHTML = generateDay2Content(dayData);
  else if (dayNum === 3) container.innerHTML = generateDay3Content(dayData);
  else if (dayNum === 4) container.innerHTML = generateDay4Content(dayData);
  else if (dayNum === 5) container.innerHTML = generateDay5Content(dayData);
}
function generateLessonCard(
  ...
  var status = isComplete ? 'completed' : isSelected ? 'current' : 'available';
  var checkContent = isComplete ? svgIcons.check : lessonIndex + 1;
  var tag = isComplete
    ? 'Complete'
    : isUpNext
      ? 'Up Next'
  ...
  var html =
    ...
    status +
    '" role="button" tabindex="0" onclick="academySelectLesson(' +
    day +
    ',' +
    lessonIndex +
    ")\" onkeydown=\"if(event.key==='Enter'||event.key===' '){event.preventDefault();academySelectLesson(" +
    day +
    ',' +
    lessonIndex +
    ');}">';
  html += '<div class="lesson-check">' + checkContent + '</div>';
  html +=
    '<div class="lesson-info"><div class="lesson-tag">' +
    tag +
    '</div><h4>' +
    title +
    '</h4><p>' +
    desc +
    '</p></div>';
  html += '<span class="lesson-time">' + time + '</span>';
  if (isSelected)
    html +=
      '<button type="button" class="lesson-action" onclick="event.stopPropagation();academySelectLesson(' +
      day +
      ',' +
      lessonIndex +
      ')">' +
...
function completeLesson(dayNum, lessonNum) {
  academyProgress['day' + dayNum].lessons[lessonNum] = true;
  var allComplete = academyProgress['day' + dayNum].lessons.every(function (l) {
    return l;
  });
  if (allComplete) {
    academyProgress['day' + dayNum].complete = true;
    showConfetti();
  }
  var lessonsAfter = academyProgress['day' + dayNum].lessons;
  var nextFi = lessonsAfter.findIndex(function (l) {
    return !l;
  });
  if (nextFi === -1) {
    academyViewLesson['d' + dayNum] = 4;
  } else {
    academyViewLesson['d' + dayNum] = nextFi;
  }
  saveAcademyProgress();
  updateAcademyUI();
  loadDayContent(dayNum);
...
function copyScript(scriptId) {
  ...
  if (!scriptEl) return;
  var text = scriptEl.innerText || scriptEl.textContent;
  navigator.clipboard.writeText(text).then(function () {
    var btn = document.querySelector('[data-copy="' + scriptId + '"]');
    if (btn) {
      btn.innerHTML = svgIcons.check + ' Copied';
      btn.style.background = '#34C759';
      setTimeout(function () {
        btn.innerHTML = svgIcons.copy + ' Copy';
...
function showConfetti() {
  var colors = ['#4A90D9', '#34C759', '#8B5CF6', '#F59E0B', '#EF4444'];
  for (var i = 0; i < 50; i++) {
...
function generateDay1Content(dayData) {
  var lessons = dayData.lessons;
  var firstIncomplete = lessons.findIndex(function (l) {
    return !l;
  });
  var viewLesson = academyGetViewLesson(1, lessons);
  var html =
    '<button class="back-btn" onclick="hideAllDayContent()">' +
    svgIcons.x +
    ' Back to all days</button>';
  html +=
    '<div class="day-header"><div class="day-header-icon">' +
    svgIcons.book +
    '</div><div><h2>Day 1: Insurance Basics</h2><p>4 quick lessons to build your foundation</p></div></div>';
  html += academyDayToneTip(1);
  html += '<div class="lessons-list">';
  html += generateLessonCard(
    1,
    0,
    'What We Sell',
    'The 3 plan types and who they help',
    '3 min',
    lessons[0],
    viewLesson === 0,
    firstIncomplete === 0 && !lessons[0]
  );
  html += generateLessonCard(
    1,
    1,
    'Key Terms',
    '7 words you will use every call',
    '5 min',
    lessons[1],
    viewLesson === 1,
    firstIncomplete === 1 && !lessons[1]
  );
  html += generateLessonCard(
    1,
    2,
  ...
  html += generateLessonCard(
    ...
    lessons[3],
    viewLesson === 3,
    firstIncomplete === 3 && !lessons[3]
  );
  html += '</div>';
  if (viewLesson === 0) html += generateLesson1_1();
  else if (viewLesson === 1) html += generateLesson1_2();
  else if (viewLesson === 2) html += generateLesson1_3();
  else if (viewLesson === 3) html += generateLesson1_4();
  else
    html +=
      '<div class="pro-tip" style="margin-top:24px;">' +
      svgIcons.check +
      '<div class="pro-tip-content"><div class="tip-label">Day 1 Complete!</div><p>Great job! Day 2 is now unlocked.</p></div></div>';
  return html;
}
function generateLesson1_1() {
  var html =
    '<div class="lesson-detail active"><div class="lesson-detail-header"><h3>What We Sell</h3></div><div class="lesson-detail-body">';
  html +=
    '<div class="intro-bubble">' +
    svgIcons.info +
    '<p>We sell 3 types of health plans. Each works differently. Here is the simple breakdown:</p></div>';
  html +=
    '<div class="academy-fixed-benefits-callout">' +
    svgIcons.info +
    '<p><strong>Fixed benefits explained:</strong> The plan pays a set dollar amount per service — not the full bill. Agent must explain this clearly on every call.</p></div>';
  html += '<div class="plan-types-grid">';
  html +=
    '<div class="plan-type-card"><div class="plan-type-header mec"><h4>MEC Plans</h4><div class="subtitle">Minimum Essential Coverage</div></div><ul class="plan-features"><li>' +
    svgIcons.check +
    ' No deductible</li><li>' +
    svgIcons.check +
    ' Copays for doctor visits</li><li>' +
    svgIcons.check +
    ' Free preventive care</li></ul><div class="best-for"><strong>Best for:</strong> Healthy people, low monthly cost</div></div>';
  html +=
    '<div class="plan-type-card"><div class="plan-type-header stm"><h4>STM Plans</h4><div class="subtitle">Short-Term Medical</div></div><ul class="plan-features"><li>' +
    svgIcons.check +
    ' Has deductible/coinsurance</li><li>' +
    svgIcons.check +
    ' Max out-of-pocket protection</li><li>' +
    svgIcons.check +
    ' Requires medical underwriting</li></ul><div class="best-for"><strong>Best for:</strong> Gap coverage, healthy people</div></div>';
  html +=
    '<div class="plan-type-card"><div class="plan-type-header indemnity"><h4>Indemnity Plans</h4><div class="subtitle">Fixed Cash Benefits</div></div><ul class="plan-features"><li>' +
    svgIcons.check +
    ' No deductible</li><li>' +
    svgIcons.check +
    ' Network repricing</li><li>' +
    svgIcons.check +
    ' Guaranteed issue</li></ul><div class="best-for"><strong>Best for:</strong> Flexibility, predictable costs</div></div>';
  html += '</div>';
  html +=
    '<div class="pro-tip">' +
    svgIcons.info +
    '<div class="pro-tip-content"><div class="tip-label">Pro Tip</div><p>Never tell customers a plan is "guaranteed issue" - it removes urgency!</p></div></div>';
  html +=
    '<button class="mark-complete-btn" onclick="completeLesson(1,0)">' +
    svgIcons.check +
    ' Got it! Mark Complete</button></div></div>';
  return html;
}
function generateLesson1_2() {
  var html =
    '<div class="lesson-detail active"><div class="lesson-detail-header"><h3>Key Terms</h3></div><div class="lesson-detail-body">';
  html +=
    '<div class="intro-bubble">' +
    svgIcons.info +
    '<p>These 7 words come up on almost every call. Know them!</p></div>';
  html += '<div class="terms-grid">';
  html +=
    '<div class="term-card"><div class="term-icon">' +
    svgIcons.dollar +
    '</div><div><h5>Premium</h5><p>Monthly cost for coverage</p></div></div>';
  html +=
    '<div class="term-card"><div class="term-icon">' +
    svgIcons.dollar +
    '</div><div><h5>Deductible</h5><p>Amount paid before insurance kicks in</p></div></div>';
  html +=
    '<div class="term-card"><div class="term-icon">' +
    svgIcons.dollar +
    '</div><div><h5>Copay</h5><p>Fixed amount per visit (like $25)</p></div></div>';
  html +=
    '<div class="term-card"><div class="term-icon">' +
    svgIcons.dollar +
    '</div><div><h5>Coinsurance</h5><p>Percentage split (80/20 = plan pays 80%)</p></div></div>';
  html +=
    '<div class="term-card"><div class="term-icon">' +
    svgIcons.shield +
    '</div><div><h5>Max Out-of-Pocket</h5><p>Most you pay in a year</p></div></div>';
  html +=
    '<div class="term-card"><div class="term-icon">' +
    svgIcons.search +
    '</div><div><h5>Network</h5><p>Doctors with contracted rates</p></div></div>';
  html +=
    '<div class="term-card"><div class="term-icon">' +
    svgIcons.alert +
    '</div><div><h5>Pre-existing (12/12)</h5><p>12-month wait for recent conditions</p></div></div>';
  html += '</div>';
  html +=
    '<button class="mark-complete-btn" onclick="completeLesson(1,1)">' +
    svgIcons.check +
    ' Got it! Mark Complete</button></div></div>';
  return html;
}
function generateLesson1_3() {
  var html =
    '<div class="lesson-detail active"><div class="lesson-detail-header"><h3>ACA vs Private Plans</h3></div><div class="lesson-detail-body">';
  html +=
    '<div class="intro-bubble">' +
    svgIcons.info +
    '<p>Customers often ask about the difference. Here is how to explain it:</p></div>';
  html += '<div class="compare-grid">';
  html +=
    '<div class="compare-card aca"><h4>ACA / Marketplace</h4><ul><li>Open Enrollment: Nov 1 - Jan 15</li><li>Need qualifying event outside OE</li><li>Subsidies if income qualifies</li><li>Covers pre-existing day 1</li><li>Covers maternity, mental health</li><li>Higher monthly cost</li></ul></div>';
  html +=
    '<div class="compare-card private"><h4>Our Private Plans</h4><ul><li>Enroll anytime</li><li>No qualifying event needed</li><li>No income requirements</li><li>12/12 pre-existing clause</li><li>No maternity, mental health</li><li>Lower monthly cost</li></ul></div>';
  html += '</div>';
  html +=
    '<div class="pro-tip">' +
    svgIcons.info +
    '<div class="pro-tip-content"><div class="tip-label">Pro Tip</div><p>Never bash ACA! Say: "ACA is great for pre-existing. Our plans are better for healthier people who want lower costs."</p></div></div>';
  html +=
    '<button class="mark-complete-btn" onclick="completeLesson(1,2)">' +
    svgIcons.check +
    ' Got it! Mark Complete</button></div></div>';
  return html;
}
function generateLesson1_4() {
  var html =
    '<div class="lesson-detail active"><div class="lesson-detail-header"><h3>Required Disclosures</h3></div><div class="lesson-detail-body">';
  html +=
    '<div class="intro-bubble">' +
    svgIcons.alert +
    '<p>You MUST say these on every call. Missing them = compliance violation!</p></div>';
  html += '<ul class="disclosure-list">';
  html +=
    '<li class="disclosure-item">' +
    svgIcons.alert +
    '<p>"This call may be recorded for training and quality assurance."</p></li>';
  html +=
    '<li class="disclosure-item">' +
    svgIcons.alert +
    '<p>"This is a private, limited-benefit plan, not ACA or major medical."</p></li>';
  html +=
    '<li class="disclosure-item">' +
    svgIcons.alert +
    '<p>"Does not cover maternity, substance abuse, or psychiatric services."</p></li>';
  html +=
    '<li class="disclosure-item">' +
    svgIcons.alert +
    '<p>"12/12 pre-existing clause - 12-month wait for recent conditions."</p></li>';
  html +=
    '<li class="disclosure-item">' +
    svgIcons.alert +
    '<p>"30-day waiting period for hospital, sickness, scheduled visits."</p></li>';
  html +=
    '<li class="disclosure-item">' +
    svgIcons.alert +
    '<p>"Must disclose network, underwriter, association, and billing."</p></li>';
  html += '</ul>';
  html +=
    '<div class="pro-tip">' +
    svgIcons.info +
    '<div class="pro-tip-content"><div class="tip-label">Pro Tip</div><p>These are built into the scripts. Just read word-for-word and you are compliant!</p></div></div>';
  html +=
    '<button class="mark-complete-btn" onclick="completeLesson(1,3)">' +
    svgIcons.check +
    ' Got it! Mark Complete</button></div></div>';
  return html;
}
function generateDay2Content(dayData) {
  var lessons = dayData.lessons;
  var firstIncomplete = lessons.findIndex(function (l) {
    return !l;
  });
  var viewLesson = academyGetViewLesson(2, lessons);
  var html =
    '<button class="back-btn" onclick="hideAllDayContent()">' +
    svgIcons.x +
    ' Back to all days</button>';
  html +=
    '<div class="day-header"><div class="day-header-icon">' +
    svgIcons.target +
    '</div><div><h2>Day 2: Pre-Qualification</h2><p>Master the opening - 4 lessons with real scripts</p></div></div>';
  html += academyDayToneTip(2);
  html += '<div class="lessons-list">';
  html += generateLessonCard(
    2,
    0,
    'Opening Script',
    'Word-for-word when they pick up',
    '4 min',
    lessons[0],
    viewLesson === 0,
    firstIncomplete === 0 && !lessons[0]
  ...
  html += generateLessonCard(
    ...
    1,
    'Prequal Questions',
    'Info you need before picking a plan',
    '5 min',
    lessons[1],
    viewLesson === 1,
    firstIncomplete === 1 && !lessons[1]
  ...
  html += generateLessonCard(
    2,
    2,
    'STM Knockout Questions',
    '15 conditions for STM eligibility',
    '3 min',
    lessons[2],
    viewLesson === 2,
    firstIncomplete === 2 && !lessons[2]
  ...
  html += '</div>';
  if (viewLesson === 0) html += generateLesson2_1();
  else if (viewLesson === 1) html += generateLesson2_2();
  else if (viewLesson === 2) html += generateLesson2_3();
  else if (viewLesson === 3) html += generateLesson2_4();
  else
    html +=
      '<div class="pro-tip" style="margin-top:24px;">' +
      svgIcons.check +
      '<div class="pro-tip-content"><div class="tip-label">Day 2 Complete!</div><p>You know the opening! Day 3 unlocked.</p></div></div>';
  return html;
}
function generateLesson2_1() {
  var html =
    '<div class="lesson-detail active"><div class="lesson-detail-header"><h3>Opening Script</h3></div><div class="lesson-detail-body">';
  html +=
    '<div class="intro-bubble">' +
    svgIcons.phone +
    '<p>This is exactly what you say when they pick up. Read it word-for-word!</p></div>';
  html +=
    '<div class="script-block"><div class="script-header"><span class="script-label">' +
    svgIcons.doc +
    ' Opening Script</span><button class="copy-btn" data-copy="openingScript" onclick="copyScript(\'openingScript\')">' +
    svgIcons.copy +
    ' Copy</button></div>';
  html +=
    '<div class="script-body" id="openingScript"><p>Central Health Advisors, this is <span class="fill-in">(FIRST AND LAST NAME)</span>. I will be your licensed health insurance agent assisting you today. Please note that this call may be recorded for training and quality assurance.</p><p>Were you looking for an individual or family plan TODAY?</p>';
  html +=
    '<div class="script-branch"><div class="branch yes"><span class="branch-label">' +
    svgIcons.check +
    ' If Individual</span><p>Move to next question</p></div><div class="branch hesitant"><span class="branch-label">If Family</span><p>How many family members will be included TODAY?</p></div></div>';
  html +=
    '<p>OK! GREAT! Now are you currently insured?</p><div class="script-callout wait">' +
    svgIcons.play +
    ' Wait for answer</div></div></div>';
  html +=
    '<div class="pro-tip">' +
    svgIcons.info +
    '<div class="pro-tip-content"><div class="tip-label">Pro Tip</div><p>Say "TODAY" with emphasis - it creates urgency!</p></div></div>';
  html +=
    '<button class="mark-complete-btn" onclick="completeLesson(2,0)">' +
    svgIcons.check +
    ' Got it! Mark Complete</button></div></div>';
  return html;
}
function generateLesson2_2() {
  var html =
    '<div class="lesson-detail active"><div class="lesson-detail-header"><h3>Prequal Questions</h3></div><div class="lesson-detail-body">';
  html +=
    '<div class="intro-bubble">' +
    svgIcons.doc +
    '<p>Gather this info to pick the right plan. Ask conversationally - do not rush!</p></div>';
  html +=
    '<div class="script-block"><div class="script-header"><span class="script-label">' +
    svgIcons.doc +
    ' Prequal Questions</span><button class="copy-btn" data-copy="prequalScript" onclick="copyScript(\'prequalScript\')">' +
    svgIcons.copy +
    ' Copy</button></div>';
  html +=
    '<div class="script-body" id="prequalScript"><p>Do you have any pre-existing medical conditions?</p><p>Are you currently taking any medications?</p><p>What is your Date of Birth?</p><p>Please verify your zip code?</p><p>Are you a tobacco user?</p><p>How often do you go to the doctors yearly?</p><p>Do you have any doctors you want to keep in network?</p><p>Any upcoming surgeries or procedures scheduled?</p><p>To check for subsidies, how much do you make yearly BEFORE TAX?</p><p>Is there a monthly price range you are hoping to stay within?</p><p>When would you like coverage to begin?</p><p>I am submitting your info now. Give me 30 to 60 seconds while I review everything.</p>';
  html +=
    '<div class="script-callout silence">' +
    svgIcons.alert +
    ' THE SIZZLE - Put on hold 30-60 sec. Builds urgency!</div></div></div>';
  html +=
    '<button class="mark-complete-btn" onclick="completeLesson(2,1)">' +
    svgIcons.check +
    ' Got it! Mark Complete</button></div></div>';
  return html;
}
function generateLesson2_3() {
  var html =
    '<div class="lesson-detail active"><div class="lesson-detail-header"><h3>STM Knockout Questions</h3></div><div class="lesson-detail-body">';
  html +=
    '<div class="intro-bubble">' +
    svgIcons.alert +
    '<p>For STM plans, you MUST ask these. YES to any = they do NOT qualify for STM!</p></div>';
  html +=
    '<div class="script-block"><div class="script-header"><span class="script-label">' +
    svgIcons.doc +
    ' 15 Knockout Conditions</span><button class="copy-btn" data-copy="knockoutScript" onclick="copyScript(\'knockoutScript\')">' +
    svgIcons.copy +
    ' Copy</button></div>';
  html +=
    '<div class="script-body" id="knockoutScript"><p>In order to determine eligibility I will read you a list of 15 conditions. Please confirm if you currently have or have ever been diagnosed with:</p>';
  html +=
    '<div class="knockout-list"><h4>' +
    svgIcons.alert +
    ' The 15 Conditions:</h4><p>Heart Disorder, Stroke, Crohns Disease, Ulcerative Colitis, Liver or Kidney Disorder, Emphysema, COPD, Cancer or Tumor, Alcohol or Drug Abuse, HIV or AIDS, Multiple Sclerosis, Tuberculosis, Autism, Schizophrenia, Lupus, Bariatric Surgery, Pregnancy?</p></div>';
  html +=
    '<div class="script-callout wait">' +
    svgIcons.play +
    ' Wait for answer - Do NOT rush!</div></div></div>';
  html +=
    '<div class="pro-tip">' +
    svgIcons.info +
    '<div class="pro-tip-content"><div class="tip-label">Pro Tip</div><p>If YES to any, say "What we can do instead is..." and pivot to MEC or Indemnity!</p></div></div>';
  html +=
    '<button class="mark-complete-btn" onclick="completeLesson(2,2)">' +
    svgIcons.check +
    ' Got it! Mark Complete</button></div></div>';
  return html;
}
function generateLesson2_4() {
  var html =
    '<div class="lesson-detail active"><div class="lesson-detail-header"><h3>Tonality Guide</h3></div><div class="lesson-detail-body">';
  html +=
    '<div class="intro-bubble">' +
    svgIcons.info +
    '<p>HOW you say it matters as much as WHAT you say. 3 tonality types:</p></div>';
  html += '<div class="tonality-cards">';
  html +=
    '<div class="tonality-card down"><h4>Breaking Rapport</h4><div class="tone-type">Down Tone</div><p>For confident statements. Voice goes DOWN at end.</p></div>';
  html +=
    '<div class="tonality-card up"><h4>Seeking Rapport</h4><div class="tone-type">Up Tone</div><p>For questions. Voice goes UP at end.</p></div>';
  html +=
    '<div class="tonality-card neutral"><h4>Neutral</h4><div class="tone-type">Monotone</div><p>For reading disclosures. Flat delivery.</p></div>';
  html += '</div>';
  html +=
    '<div class="script-block"><div class="script-header"><span class="script-label">' +
    svgIcons.doc +
    ' Key Rules</span></div><div class="script-body">';
  html +=
    '<p>Mirror customer energy +5-10%</p><p>Speak like a professional, not a friend</p><p>Stop overtalking - say your piece and STOP</p><p>Never say "basically," "actually," "honestly," "um/uh"</p><p>When in doubt, pause instead of filling with words</p></div></div>';
  html +=
    '<div class="pro-tip">' +
    svgIcons.info +
    '<div class="pro-tip-content"><div class="tip-label">Pro Tip</div><p>Silence sounds confident! Just pause - do NOT fill with "um."</p></div></div>';
  html +=
    '<button class="mark-complete-btn" onclick="completeLesson(2,3)">' +
    svgIcons.check +
    ' Got it! Mark Complete</button></div></div>';
  return html;
}
function generateDay3Content(dayData) {
  var lessons = dayData.lessons;
  var firstIncomplete = lessons.findIndex(function (l) {
    return !l;
  });
  var viewLesson = academyGetViewLesson(3, lessons);
  var html =
    '<button class="back-btn" onclick="hideAllDayContent()">' +
    svgIcons.x +
    ' Back to all days</button>';
  html +=
    '<div class="day-header"><div class="day-header-icon">' +
    svgIcons.chat +
    '</div><div><h2>Day 3: The Pitch</h2><p>Present plans with confidence</p></div></div>';
  html += academyDayToneTip(3);
  html += '<div class="lessons-list">';
  html += generateLessonCard(
    3,
    0,
    'Return from Hold',
    'Coming back with the SIZZLE',
    '3 min',
    lessons[0],
    viewLesson === 0,
    firstIncomplete === 0 && !lessons[0]
  ...
  html += '</div>';
  if (viewLesson === 0) html += generateLesson3_1();
  else if (viewLesson === 1) html += generateLesson3_2();
  else if (viewLesson === 2) html += generateLesson3_3();
  else if (viewLesson === 3) html += generateLesson3_4();
  else
    html +=
      '<div class="pro-tip" style="margin-top:24px;">' +
      svgIcons.check +
      '<div class="pro-tip-content"><div class="tip-label">Day 3 Complete!</div><p>You can pitch! Day 4 unlocked.</p></div></div>';
  return html;
}
function generateLesson3_1() {
  var html =
    '<div class="lesson-detail active"><div class="lesson-detail-header"><h3>Return from Hold</h3></div><div class="lesson-detail-body">';
  html +=
    '<div class="script-callout dont-wait">' +
    svgIcons.x +
    ' DO NOT ask "are you still there?" - Start speaking immediately!</div>';
  html +=
    '<div class="script-block"><div class="script-header"><span class="script-label">' +
    svgIcons.doc +
    ' Teaser Script</span><button class="copy-btn" data-copy="teaserScript" onclick="copyScript(\'teaserScript\')">' +
    svgIcons.copy +
    ' Copy</button></div>';
  html +=
    '<div class="script-body" id="teaserScript"><p>Alright <span class="fill-in">[Customer Name]</span>, I have some excellent news for you. Based on the information you provided, I was able to locate a plan for you through the <span class="fill-in">[Network Name]</span>.</p><p>This plan offers fixed benefits for doctor visits, hospital care, and prescription savings. You have flexibility to see any provider within the network, no referrals needed for specialists.</p><p>Another key advantage is nationwide access. And most importantly, there is no deductible.</p><p>Does everything sound clear so far?</p><div class="script-callout wait">' +
    svgIcons.play +
    ' Wait for answer</div></div></div>';
  html +=
    '<div class="pro-tip">' +
    svgIcons.info +
    '<div class="pro-tip-content"><div class="tip-label">Pro Tip</div><p>Come back at HIGHER energy! Speak at a steady pace they cannot interrupt.</p></div></div>';
  html +=
    '<button class="mark-complete-btn" onclick="completeLesson(3,0)">' +
    svgIcons.check +
    ' Got it! Mark Complete</button></div></div>';
  return html;
}
function generateLesson3_2() {
  var html =
    '<div class="lesson-detail active"><div class="lesson-detail-header"><h3>Price Presentation</h3></div><div class="lesson-detail-body">';
  html +=
    '<div class="intro-bubble">' +
    svgIcons.dollar +
    '<p>After the teaser, present the price. Include enrollment fee in total. Make it affordable!</p></div>';
  html +=
    '<div class="script-block"><div class="script-header"><span class="script-label">' +
    svgIcons.doc +
    ' Price Script</span><button class="copy-btn" data-copy="priceScript" onclick="copyScript(\'priceScript\')">' +
    svgIcons.copy +
    ' Copy</button></div>';
  html +=
    '<div class="script-body" id="priceScript"><p>Great. Based on everything we reviewed, the monthly rate comes out to <span class="fill-in">$___</span>.</p><p>There is a one-time standard group association fee of <span class="fill-in">$___</span> to activate your membership.</p><p>That brings the total for the first month to <span class="fill-in">$___</span>, and then moving forward, your monthly cost is simply <span class="fill-in">$___</span> thereafter.</p><p>Now, assuming this plan meets your medical needs, would the initial first-month amount be affordable for you today?</p><div class="script-callout wait">' +
    svgIcons.play +
    ' Wait for answer</div></div></div>';
  html +=
    '<div class="pro-tip">' +
    svgIcons.info +
    '<div class="pro-tip-content"><div class="tip-label">Pro Tip</div><p>If they gave budget of $200, present at $180-190. Just under = no-brainer!</p></div></div>';
  html +=
    '<button class="mark-complete-btn" onclick="completeLesson(3,1)">' +
    svgIcons.check +
    ' Got it! Mark Complete</button></div></div>';
  return html;
}
function generateLesson3_3() {
  var html =
    '<div class="lesson-detail active"><div class="lesson-detail-header"><h3>Benefits Script</h3></div><div class="lesson-detail-body">';
  html +=
    '<div class="intro-bubble">' +
    svgIcons.doc +
    '<p>Once they agree price works, walk through benefits. Then RX, then closing - do NOT pause!</p></div>';
  html +=
    '<div class="script-block"><div class="script-header"><span class="script-label">' +
    svgIcons.doc +
    ' Benefits + RX + Closing</span><button class="copy-btn" data-copy="benefitsScript" onclick="copyScript(\'benefitsScript\')">' +
    svgIcons.copy +
    ' Copy</button></div>';
  html +=
    '<div class="script-body" id="benefitsScript"><p>Perfect. What I will do next is walk you through the full benefits. If everything looks good, we will submit the application.</p><p>During verification, you will receive an email with coverage outline and digital insurance cards. Physical cards arrive by mail in 1-3 weeks. Okay?</p><div class="script-callout wait">' +
    svgIcons.play +
    ' Wait for answer</div>';
  html +=
    '<p>You do not need referrals to see a specialist. When you visit a doctor, the bill is first reduced to the network contracted rate, then your benefit is applied.</p><p>People like this plan because it is simple - no deductible, pays defined benefit amounts.</p>';
  html +=
    '<div class="script-callout dont-wait">' +
    svgIcons.x +
    ' DO NOT WAIT - Go straight to RX!</div>';
  html +=
    '<p><strong>RX:</strong> You will also receive access to a prescription discount program at major pharmacies.</p><p>The plan does not include benefits for mental health, substance abuse, or pregnancy-related care. You will not be needing coverage for those services, correct?</p>';
  html +=
    '<div class="script-callout dont-wait">' +
    svgIcons.x +
    ' DO NOT WAIT - Go to Closing Statement!</div></div></div>';
  html +=
    '<button class="mark-complete-btn" onclick="completeLesson(3,2)">' +
    svgIcons.check +
    ' Got it! Mark Complete</button></div></div>';
  return html;
}
function generateLesson3_4() {
  var html =
    '<div class="lesson-detail active"><div class="lesson-detail-header"><h3>Plan Lookup</h3></div><div class="lesson-detail-body">';
  html +=
    '<div class="intro-bubble">' +
    svgIcons.search +
    '<p>Use this to find network, underwriter, and provider search URL for any plan.</p></div>';
  html +=
  ...
  html +=
    '<div class="pro-tip">' +
    svgIcons.info +
    '<div class="pro-tip-content"><div class="tip-label">Pro Tip</div><p>Have this open during calls! You need the network and provider URL for post-close.</p></div></div>';
  html +=
    '<button class="mark-complete-btn" onclick="completeLesson(3,3)">' +
    svgIcons.check +
    ' Got it! Mark Complete</button></div></div>';
  return html;
}
function showPlanInfo() {
  ...
  var display = document.getElementById('planInfoDisplay');
  var planKey = select.value;
  if (!planKey || !planReferenceData[planKey]) {
    display.innerHTML = '';
    return;
  }
  var plan = planReferenceData[planKey];
  var html =
    '<div class="plan-info-card"><h4>' +
    plan.name +
    '</h4><div class="plan-info-grid">';
  html +=
    '<div class="plan-info-item"><span>Network</span><strong>' +
    plan.network +
    '</strong></div>';
  if (plan.claimsAdmin)
    html +=
      '<div class="plan-info-item"><span>Claims Admin</span><strong>' +
      plan.claimsAdmin +
      '</strong></div>';
  if (plan.underwriter)
    html +=
      '<div class="plan-info-item"><span>Underwriter</span><strong>' +
      plan.underwriter +
      '</strong></div>';
  html +=
    '<div class="plan-info-item"><span>Association</span><strong>' +
    plan.association +
    '</strong></div>';
  html +=
    '<div class="plan-info-item"><span>Type</span><strong>' +
    plan.type +
    '</strong></div>';
  if (plan.providerSearch)
    html +=
      '<div class="plan-info-item"><span>Provider Search</span><strong>' +
      plan.providerSearch +
      '</strong></div>';
  html += '</div></div>';
  display.innerHTML = html;
}
function generateDay4Content(dayData) {
  var lessons = dayData.lessons;
  var firstIncomplete = lessons.findIndex(function (l) {
    return !l;
  });
  var viewLesson = academyGetViewLesson(4, lessons);
  var html =
    '<button class="back-btn" onclick="hideAllDayContent()">' +
    svgIcons.x +
    ' Back to all days</button>';
  html +=
    ...
    '</div><div><h2>Day 4: Close & Verify</h2><p>Seal the deal - 4 lessons</p></div></div>';
  html += academyDayToneTip(4);
  html += '<div class="lessons-list">';
  html += generateLessonCard(
    4,
    0,
    'Collect Info',
    'Email, address, phone confirmation',
    '3 min',
    lessons[0],
    viewLesson === 0,
    firstIncomplete === 0 && !lessons[0]
  ...
  html += generateLessonCard(
    ...
    1,
    'Payment Collection',
    'Get the card - then STOP TALKING',
    '3 min',
    lessons[1],
    viewLesson === 1,
    firstIncomplete === 1 && !lessons[1]
  ...
  html += '</div>';
  if (viewLesson === 0) html += generateLesson4_1();
  else if (viewLesson === 1) html += generateLesson4_2();
  else if (viewLesson === 2) html += generateLesson4_3();
  else if (viewLesson === 3) html += generateLesson4_4();
  else
    html +=
      '<div class="pro-tip" style="margin-top:24px;">' +
      svgIcons.check +
      '<div class="pro-tip-content"><div class="tip-label">Day 4 Complete!</div><p>You know how to close! Final day unlocked.</p></div></div>';
  return html;
}
function generateLesson4_1() {
  var html =
    '<div class="lesson-detail active"><div class="lesson-detail-header"><h3>Collect Info</h3></div><div class="lesson-detail-body">';
  html +=
    '<div class="script-block"><div class="script-header"><span class="script-label">' +
    svgIcons.doc +
    ' Info Collection</span><button class="copy-btn" data-copy="infoScript" onclick="copyScript(\'infoScript\')">' +
    svgIcons.copy +
    ' Copy</button></div>';
  html +=
    '<div class="script-body" id="infoScript"><p>OK Perfect, what is a good email we can send all this over to?</p><p>Would you like your cards issued with a middle initial, or just first and last name?</p><p>What is the physical mailing address for hard-copy documents?</p><p>Is this phone number <span class="fill-in">________</span> the best number for the insurance company to reach you?</p><p>Before we complete the enrollment, I do need to verify a few items on this recorded line. Can you please confirm that you are a United States citizen or legal resident, and that all health information you provided today is accurate and complete to the best of your knowledge? Lastly, for identity verification purposes, please confirm your Social Security number.</p><div class="script-callout wait">' +
    svgIcons.play +
    ' Wait for answer</div></div></div>';
  html +=
    '<button class="mark-complete-btn" onclick="completeLesson(4,0)">' +
    svgIcons.check +
    ' Got it! Mark Complete</button></div></div>';
  return html;
}
function generateLesson4_2() {
  var html =
    '<div class="lesson-detail active"><div class="lesson-detail-header"><h3>Payment Collection</h3></div><div class="lesson-detail-body">';
  html +=
    '<div class="intro-bubble">' +
    svgIcons.card +
    '<p>This is the most critical moment. Get the card, then STOP TALKING. Whoever speaks first loses!</p></div>';
  html +=
    '<div class="script-block"><div class="script-header"><span class="script-label">' +
    svgIcons.doc +
    ' Payment Script</span><button class="copy-btn" data-copy="paymentScript" onclick="copyScript(\'paymentScript\')">' +
    svgIcons.copy +
    ' Copy</button></div>';
  html +=
    '<div class="script-body" id="paymentScript"><p>For the initial payment, which card type would you prefer to put on file?</p><div class="script-callout wait">' +
    svgIcons.play +
    ' Wait for answer</div><p>Ok go ahead with the card number.</p><div class="script-callout silence">' +
    svgIcons.x +
    ' DO NOT SPEAK - WHOEVER TALKS FIRST LOSES</div></div></div>';
  html +=
    '<div class="pro-tip">' +
    svgIcons.info +
    '<div class="pro-tip-content"><div class="tip-label">Pro Tip</div><p>MUTE yourself if you have to. The silence feels awkward but it WORKS!</p></div></div>';
  html +=
    '<button class="mark-complete-btn" onclick="completeLesson(4,1)">' +
    svgIcons.check +
    ' Got it! Mark Complete</button></div></div>';
  return html;
}
function generateLesson4_3() {
  var html =
    '<div class="lesson-detail active"><div class="lesson-detail-header"><h3>Verification Walkthrough</h3></div><div class="lesson-detail-body">';
  html +=
    '<div class="script-block"><div class="script-header"><span class="script-label">' +
    svgIcons.doc +
    ' Verification Script</span><button class="copy-btn" data-copy="verifyScript" onclick="copyScript(\'verifyScript\')">' +
    svgIcons.copy +
    ' Copy</button></div>';
  html +=
    '<div class="script-body" id="verifyScript"><p>This is the final step. I am sending you a text now from a 732 number labeled "ONE ENROLLMENT." Please click the link once you receive it. Let me know when you see it.</p><div class="script-callout wait">' +
    svgIcons.play +
    ' Wait for answer</div><p>Great. Please open the link and, if possible, place your phone on speaker so I can walk you through.</p><p>Your role is to confirm everything is correct - spelling of name, address, email, phone, date of birth, gender, payment method.</p><p>Once you have reviewed, let me know - does it all look correct?</p><div class="script-callout wait">' +
    svgIcons.play +
    ' Wait for answer</div><p>At the bottom, enter your first and last name - please capitalize both. Sign electronically with your finger. Click "Accept."</p></div></div>';
  html +=
    '<div class="script-block"><div class="script-header"><span class="script-label">' +
    svgIcons.doc +
    ' Tape Confirmation Script</span><button class="copy-btn" data-copy="tapeConfirmScript" onclick="copyScript(\'tapeConfirmScript\')">' +
    svgIcons.copy +
    ' Copy</button></div>';
  html +=
    '<div class="script-body" id="tapeConfirmScript"><p><strong>SAY VERBATIM:</strong> "Before we complete the enrollment, I do need to verify a few items on this recorded line. Can you please confirm that you are a United States citizen or legal resident, and that all health information you provided today is accurate and complete to the best of your knowledge? Lastly, for identity verification purposes, please confirm your Social Security number."</p></div></div>';
  html +=
    '<button class="mark-complete-btn" onclick="completeLesson(4,2)">' +
    svgIcons.check +
    ' Got it! Mark Complete</button></div></div>';
  return html;
}
...
function generateDay5Content(dayData) {
  ...
  html +=
    '<div class="day-header"><div class="day-header-icon">' +
    svgIcons.rocket +
    '</div><div><h2>Day 5: Go Live Ready</h2><p>Final prep before your first call</p></div></div>';
  html += academyDayToneTip(5);
  html += '<div class="lessons-list">';
  html += generateLessonCard(
    5,
    0,
    'Common Objections',
    'What they say and how to respond',
    '6 min',
    lessons[0],
    viewLesson === 0,
    firstIncomplete === 0 && !lessons[0]
  ...
  html += generateLessonCard(
    ...
    2,
    'Full Call Flow',
    'Visual overview of entire call',
    '4 min',
    lessons[2],
    viewLesson === 2,
    firstIncomplete === 2 && !lessons[2]
  ...
  html += '</div>';
  if (viewLesson === 0) html += generateLesson5_1();
  else if (viewLesson === 1) html += generateLesson5_2();
  else if (viewLesson === 2) html += generateLesson5_3();
  else if (viewLesson === 3) html += generateLesson5_4();
  else
    html +=
      '<div class="pro-tip" style="margin-top:24px;">' +
      svgIcons.check +
      '<div class="pro-tip-content"><div class="tip-label">Academy Complete!</div><p>You are READY! Time to make your first sale. Read scripts word-for-word until natural!</p></div></div>';
  return html;
}
function generateLesson5_1() {
  var html =
    '<div class="lesson-detail active"><div class="lesson-detail-header"><h3>Common Objections</h3></div><div class="lesson-detail-body">';
  html +=
    '<div class="intro-bubble">' +
    svgIcons.shield +
    '<p>Customers will push back. Click each objection to see the response:</p></div>';
  html += '<div class="objection-cards">';
  var objections = [
    {
      q: 'How much will the doctor visit cost me?',
      a: 'Harmony Care pays fixed cash benefits based on plan limits. The benefit helps offset costs, not cover them in full.'
    },
    {
      q: 'Is there a copay?',
      a: 'No traditional copay. Instead, the plan pays a defined cash benefit per visit. This keeps monthly costs lower.'
    },
    {
      q: 'Can I receive this in email?',
      a: 'Yes, I will send you the terms and conditions in the verification E-Signature you accept before enrolling.'
    },
    {
      q: 'Is this a PPO or HMO?',
      a: 'These plans use a PPO NETWORK. No referrals needed for specialists, but you must be in-network for benefits.'
    },
    {
      q: 'What is the difference vs ACA?',
      a: 'STM has waiting periods on pre-existing, no maternity/mental health, can be held up to 2 years. Lower cost for healthy people.'
    },
    {
    ...
    {
      q: 'Wait for Open Enrollment',
      a: 'A lot of people say that — the concern is just the gap. Between now and then, if something happens, you are fully exposed.'
    }
  ];
  for (var i = 0; i < objections.length; i++) {
    html +=
      '<div class="objection-card" onclick="this.classList.toggle(\'open\')"><div class="objection-header"><h4>' +
      svgIcons.shield +
      ' "' +
      objections[i].q +
      '"</h4><span class="toggle">▼</span></div><div class="objection-body"><p>' +
      objections[i].a +
      '</p></div></div>';
  }
  html += '</div>';
  html +=
    '<button class="mark-complete-btn" onclick="completeLesson(5,0)">' +
    svgIcons.check +
    ' Got it! Mark Complete</button></div></div>';
  return html;
}
function generateLesson5_2() {
  var html =
    '<div class="lesson-detail active"><div class="lesson-detail-header"><h3>Never Say List</h3></div><div class="lesson-detail-body">';
  html +=
    '<div class="intro-bubble">' +
    svgIcons.x +
    '<p>These words will hurt your sales. Avoid them at all costs!</p></div>';
  html += '<div class="never-say-grid">';
  var neverSay = [
    'Basically...',
    'Actually...',
    'Honestly...',
  ...
  for (var i = 0; i < neverSay.length; i++) {
    html +=
      '<div class="never-say-item">' +
      svgIcons.x +
      '<p>"' +
      neverSay[i] +
      '"</p></div>';
  ...
  html +=
    '<div class="script-block"><div class="script-header"><span class="script-label">' +
    svgIcons.check +
    ' Say This Instead</span></div><div class="script-body">';
  html +=
    '<p><strong>Instead of "You dont qualify":</strong> "What we can do instead is..."</p>';
  html +=
    '<p><strong>Instead of "Um/Uh":</strong> Just pause. Silence sounds confident!</p>';
  html +=
    '<p><strong>Instead of asking after hold:</strong> Start speaking immediately with energy!</p></div></div>';
  html +=
    '<button class="mark-complete-btn" onclick="completeLesson(5,1)">' +
    svgIcons.check +
    ' Got it! Mark Complete</button></div></div>';
  return html;
}
function generateLesson5_3() {
  var html =
    '<div class="lesson-detail active"><div class="lesson-detail-header"><h3>Full Call Flow</h3></div><div class="lesson-detail-body">';
  html +=
    '<div class="intro-bubble">' +
    svgIcons.phone +
    '<p>The entire call from start to finish. Each step flows into the next!</p></div>';
  html += '<div class="call-flow-visual">';
  var steps = [
    ['1. Opening', 'Introduce yourself, ask individual or family'],
    ['2. Currently Insured?', 'Find their current situation'],
    ['3. Prequal Questions', 'Gather medical info, DOB, zip, budget'],
    ['4. The Sizzle', 'Put on hold 30-60 seconds'],
    ['5. Teaser', 'Come back with energy, present plan'],
    ['6. Price', 'Monthly + enrollment fee = first month total'],
    ['7. Benefits', 'Walk through coverage, RX, exclusions'],
    ['8. Closing Statement', 'Confirm understanding'],
    ['9. Collect Info', 'Email, address, SSN verification'],
    ['10. Payment', 'Get card number - DO NOT SPEAK'],
    ['11. Verification', 'Walk through DocuSign'],
    ['12. Post-Close', 'CS number, confirmation, network info']
  ];
  for (var i = 0; i < steps.length; i++) {
    html +=
      '<div class="flow-step"><div class="flow-line"><div class="flow-dot"></div>';
    if (i < steps.length - 1) html += '<div class="flow-connector"></div>';
    html +=
      '</div><div class="flow-content"><h4>' +
      steps[i][0] +
      '</h4><p>' +
      steps[i][1] +
      '</p></div></div>';
  ...
  html += '</div>';
  html +=
    '<button class="mark-complete-btn" onclick="completeLesson(5,2)">' +
    svgIcons.check +
    ' Got it! Mark Complete</button></div></div>';
  return html;
}
function generateLesson5_4() {
  var html =
    '<div class="lesson-detail active"><div class="lesson-detail-header"><h3>Mock Call Checklist</h3></div><div class="lesson-detail-body">';
  html +=
    '<div class="intro-bubble">' +
    svgIcons.doc +
    '<p>Before going live, practice and check off each item:</p></div>';
  ...
  for (var i = 0; i < items.length; i++) {
    html +=
      '<div class="checklist-item" onclick="this.classList.toggle(\'checked\');this.querySelector(\'input\').checked=this.classList.contains(\'checked\')"><input type="checkbox"><label>' +
      items[i] +
      '</label></div>';
  }
  html += '</div>';
  html +=
    '<div class="pro-tip">' +
    svgIcons.rocket +
    '<div class="pro-tip-content"><div class="tip-label">You Are Ready!</div><p>Once all boxes checked, you are ready for your first live call. Read scripts word-for-word until natural!</p></div></div>';
  html +=
    '<button class="mark-complete-btn" onclick="completeLesson(5,3)">' +
    svgIcons.check +
    ' Complete Academy!</button></div></div>';
  return html;
}
function toggleBenefitsPanel() {
  var panel = document.getElementById('benefitsPanel');
  var btn = document.getElementById('benefitsBubbleBtn');
  panel.classList.toggle('open');
  btn.classList.toggle('active');
  if (panel.classList.contains('open')) {
    btn.innerHTML = svgIcons.x;
    loadBenefitsContent('scripts');
  } else {
    btn.innerHTML = svgIcons.book;
  }
}
function switchBenefitsTab(tabId) {
  document.querySelectorAll('.benefits-tab').forEach(function (tab) {
    tab.classList.remove('active');
  });
  event.target.closest('.benefits-tab').classList.add('active');
  loadBenefitsContent(tabId);
}
function initBenefitsPanel() {
  loadBenefitsContent('scripts');
}
function loadBenefitsContent(tabId) {
  var content = document.getElementById('benefitsContent');
  if (!content) return;
  var html = '';
  if (tabId === 'scripts') {
    html +=
      '<div class="ref-card"><div class="ref-card-header"><span class="ref-card-title">' +
      svgIcons.phone +
      ' Opening</span><button class="ref-copy-btn" onclick="copyRefScript(\'refOpening\')">' +
      svgIcons.copy +
      ' Copy</button></div><div class="ref-card-body" id="refOpening"><p>Central Health Advisors, this is <span class="ref-highlight">(NAME)</span>. I will be your licensed agent...</p><p>Were you looking for individual or family plan TODAY?</p></div></div>';
    html +=
      '<div class="ref-card"><div class="ref-card-header"><span class="ref-card-title">' +
      svgIcons.dollar +
      ' Price</span><button class="ref-copy-btn" onclick="copyRefScript(\'refPrice\')">' +
      svgIcons.copy +
      ' Copy</button></div><div class="ref-card-body" id="refPrice"><p>Monthly rate: <span class="ref-highlight">$___</span>. One-time fee: <span class="ref-highlight">$___</span>. Total first month: <span class="ref-highlight">$___</span>.</p></div></div>';
    html +=
      '<div class="ref-card"><div class="ref-card-header"><span class="ref-card-title">' +
      svgIcons.card +
      ' Payment</span><button class="ref-copy-btn" onclick="copyRefScript(\'refPayment\')">' +
      svgIcons.copy +
      ' Copy</button></div><div class="ref-card-body" id="refPayment"><p>Which card type would you prefer?</p><p style="background:#1a1a2e;color:white;padding:8px;border-radius:6px;text-align:center;margin-top:8px;">' +
      svgIcons.x +
      ' DO NOT SPEAK - WHOEVER TALKS FIRST LOSES</p></div></div>';
    html +=
      '<div class="ref-card"><div class="ref-card-header"><span class="ref-card-title">' +
      svgIcons.check +
      ' Post-Close</span><button class="ref-copy-btn" onclick="copyRefScript(\'refPostClose\')">' +
      svgIcons.copy +
      ' Copy</button></div><div class="ref-card-body" id="refPostClose"><p>CS: <strong>855-736-1590</strong> (M-F 9am-9pm ET)</p><p>Confirmation#: <strong>CHA561337</strong></p></div></div>';
  } else if (tabId === 'plans') {
    html +=
    ...
    var objs = [
      {
        q: 'How much will doctor visit cost?',
        a: 'Fixed cash benefits based on plan limits. Helps offset costs, not cover in full.'
      },
      {
        q: 'Is there a copay?',
        a: 'No traditional copay. Plan pays defined cash benefit per visit.'
      },
      {
        q: 'I need to think about it',
        a: 'What specific concerns can I address right now?'
      },
      {
        q: 'Is this PPO or HMO?',
        a: 'PPO NETWORK. No referrals needed, must be in-network for benefits.'
      },
      {
        q: 'How do you get paid?',
        a: 'I work for Central Health Advisors and receive a weekly salary.'
      }
    ];
    for (var i = 0; i < objs.length; i++) {
      html +=
        '<div class="ref-objection-item" onclick="this.classList.toggle(\'open\')"><h4>' +
        svgIcons.shield +
        ' "' +
        objs[i].q +
        '"</h4><p>' +
  ...
  } else if (tabId === 'numbers') {
    html +=
      '<div class="ref-number-grid"><div class="ref-number-card"><div class="label">Customer Service</div><div class="value">855-736-1590</div></div><div class="ref-number-card"><div class="label">Hours</div><div class="value">M-F 9am-9pm ET</div></div><div class="ref-number-card"><div class="label">Confirmation #</div><div class="value">CHA561337</div></div><div class="ref-number-card"><div class="label">Verification SMS</div><div class="value">732 Number</div></div></div>';
    html +=
      '<div class="ref-card" style="margin-top:16px;"><div class="ref-card-title" style="margin-bottom:10px;">' +
      svgIcons.alert +
      ' Required Disclosures</div><div class="ref-card-body" style="font-size:13px;"><p>' +
      svgIcons.check +
      ' "Call may be recorded for training"</p><p>' +
      svgIcons.check +
      ' "Private, limited-benefit plan, not ACA"</p><p>' +
      svgIcons.check +
      ' "Does not cover maternity, substance abuse, psychiatric"</p><p>' +
      svgIcons.check +
      ' "12/12 pre-existing + 30-day waiting period"</p></div></div>';
    html +=
      '<div class="ref-card" style="background:#FEF3C7;"><div class="ref-card-title" style="margin-bottom:10px;">' +
      svgIcons.x +
      ' Never Say</div><div class="ref-card-body" style="font-size:13px;"><p>' +
      svgIcons.x +
      ' "Basically" / "Actually" / "Honestly"</p><p>' +
      svgIcons.x +
      ' "Um" / "Uh" (just pause)</p><p>' +
      svgIcons.x +
      ' "You dont qualify"</p><p>' +
      svgIcons.x +
      ' "Guaranteed issue"</p></div></div>';
  }
  content.innerHTML = html;
}
function showRefPlanInfo() {
  var select = document.getElementById('refPlanSelect');
  var display = document.getElementById('refPlanDisplay');
  var planKey = select.value;
  if (!planKey || !planReferenceData[planKey]) {
    display.innerHTML = '';
    return;
  ...
  var plan = planReferenceData[planKey];
  var html =
    '<div class="ref-plan-info"><p><strong>Network:</strong> ' +
    plan.network +
    '</p>';
  if (plan.claimsAdmin)
    html += '<p><strong>Claims Admin:</strong> ' + plan.claimsAdmin + '</p>';
  if (plan.underwriter)
    html += '<p><strong>Underwriter:</strong> ' + plan.underwriter + '</p>';
  html += '<p><strong>Association:</strong> ' + plan.association + '</p>';
  if (plan.providerSearch)
    html +=
      '<p><strong>Provider Search:</strong> ' + plan.providerSearch + '</p>';
  html += '</div>';
  display.innerHTML = html;
}
function copyRefScript(scriptId) {
...
document.addEventListener('DOMContentLoaded', function () {
  if (document.getElementById('academyContainer')) {
    initAcademy();
  }
});
