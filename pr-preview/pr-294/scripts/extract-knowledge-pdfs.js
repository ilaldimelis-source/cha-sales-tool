...
async function main() {
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.log(
        'OK (' + text.length + ' chars, ' + (data.numpages || '?') + ' pages)'
      );
    } catch (e) {
    ...
    if (srcKey) {
      ...
      lines.push(
        '- **No matching plan `source`** → orphan POLICY_DOCS entry `' +
          slugId(file) +
          '`'
      );
    }
  ...
  const orphanPlansJson = JSON.stringify(
    orphans.map((o) => minimalOrphanPlan(o.file, o.text))
  );
