...
function _stHandleRatesActionClick(btn) {
  ...
  if (action === 'recalc-toggle-reversals') {
    if (_stRecalcPreview && !_stRecalcPreview.applied) {
      _stRecalcPreview.includeReversals = !_stRecalcPreview.includeReversals;
      _stRender();
    }
    return;
  }
...
function _stPushRecalcLine(list, s, stored, next, bonus) {
  ...
  if (sale.type === 'deal') {
    ...
    if (sale.receiptId && sales && sales.length) {
      ...
      for (ai = 0; ai < sales.length; ai++) {
        ...
        if (String(ad.receiptId || '') !== String(sale.receiptId || ''))
          continue;
        linked = true;
...
function _stBuildRecalcPreview(sales) {
  ...
  for (i = 0; i < (sales || []).length; i++) {
    ...
    if (_stIsReversalStatus(s)) {
      ...
      _stPushRecalcLine(reversals, s, stored, next, bonus);
      continue;
    }
    ...
    _stPushRecalcLine(items, s, stored, next, bonus);
  }
  var net = 0;
  var bonusNet = 0;
  var reversalNet = 0;
  ...
  return {
    ...
    reversalNet: _stMoney2(reversalNet),
    updated: 0
  };
}
...
function _stApplyRecalcPreview() {
  ...
  var includeRev = !!preview.includeReversals;
  var hasItems = preview.items && preview.items.length;
  var hasRevs = includeRev && preview.reversals && preview.reversals.length;
  if (!hasItems && !hasRevs) {
  ...
  if (hasRevs) {
    for (i = 0; i < preview.reversals.length; i++) {
      if (preview.reversals[i] && preview.reversals[i].saleId) {
        reversalIds[preview.reversals[i].saleId] = true;
        ids[preview.reversals[i].saleId] = true;
      }
    }
  }
  var updated = 0;
  for (i = 0; i < sales.length; i++) {
    ...
    if (_stIsReversalStatus(s)) {
      if (!reversalIds[String(s.id || '')]) continue;
      if (_stReversalPaycheckWeekIsVerified(s)) continue;
      if (s.reversal && typeof s.reversal === 'object') {
        if (s.reversal.amountLostBeforeRecalc == null) {
          s.reversal.amountLostBeforeRecalc = s.reversal.amountLost;
        }
        if (s.reversal.originalCommissionBeforeRecalc == null) {
          s.reversal.originalCommissionBeforeRecalc =
            s.reversal.originalCommission;
        }
      }
    } else if (_stPaycheckWeekIsVerified(s.ts)) {
      continue;
    }
    ...
    if (s.type === 'addon') {
      ...
      if (_stIsReversalStatus(s)) {
        s.addonCommission = Math.abs(Number(s.addonCommission) || 0);
      }
    } else {
      if (
  ...
  for (i = 0; i < sales.length; i++) {
    if (sales[i] && reversalIds[String(sales[i].id || '')]) {
      _stRefreshSaleReversalAmount(sales[i]);
    }
  }
  _stSaveSales(sales);
  _stRecalcPreview = {
    applied: true,
    items: preview.items,
    reversals: preview.reversals || [],
    includeReversals: includeRev,
    skipped: preview.skipped || [],
    net: preview.net,
    bonusNet: preview.bonusNet,
    reversalNet: preview.reversalNet,
    updated: updated
  };
  var netChange = preview.net + preview.bonusNet;
  if (includeRev) netChange += Number(preview.reversalNet) || 0;
  _stFlash(
    'Updated ' +
      updated +
      ' sales. Skipped ' +
...
function _stBuildRecalcLineRowsHtml(list) {
  var html = '';
  var i;
  for (i = 0; i < (list || []).length; i++) {
    var it = list[i];
...
function _stBuildRecalcPreviewHtml(preview) {
  ...
  var hasItems = preview.items && preview.items.length;
  var hasRevs = preview.reversals && preview.reversals.length;
  if (!hasItems && !hasRevs) {
    html +=
  ...
  if (hasItems) {
    ...
    html += _stBuildRecalcLineRowsHtml(preview.items);
    html += '</tbody></table></div>';
  }
  if (hasRevs) {
    html += '<div class="st-recalc-reversals">';
    html +=
      '<div class="st-recalc-title">Chargebacks and cancels logged at old rates</div>';
    html +=
      '<p class="st-rates-help">These reversals are separate from ordinary sales. Include them only if the original sale was logged at the wrong rate. Verified paycheck weeks stay frozen.</p>';
    html += '<div class="st-rates-table-wrap"><table class="st-rates-table">';
    html +=
      '<thead><tr><th>Customer</th><th>Product</th><th>Stored</th><th>New</th><th>Difference</th></tr></thead><tbody>';
    html += _stBuildRecalcLineRowsHtml(preview.reversals);
    html += '</tbody></table></div>';
    html +=
      '<button type="button" class="st-recalc-reversals-toggle' +
      (preview.includeReversals ? ' is-on' : '') +
      '" data-st-rates-action="recalc-toggle-reversals" aria-pressed="' +
      (preview.includeReversals ? 'true' : 'false') +
      '">' +
      (preview.includeReversals
        ? 'Included in this recalculation (click to exclude)'
        : 'Excluded from this recalculation (click to include)') +
      '</button>';
    html += '</div>';
  }
...
function _stSaleQualifiesEnrollmentBonus(sale, sales) {
  ...
  var fee = Number(sale.enrollmentFee);
  if (fee !== 125 && Number(sale.enrollmentAmount) !== 125) return false;
  // Same-week cancels and chargebacks both reverse the sale in
  // tracker data. The $20 is paid only when the deal still stands
  // that week. `cancel` normalizes to chargeback, so excluding
  // only `samecancel` left one extra $20 on weeks that stored a
  // same-week reversal as a chargeback.
  if (_stIsReversalStatus(sale)) return false;
...
function _stUpdateStatus(id, newStatus) {
  ...
  for (var i = 0; i < sales.length; i++) {
    if (sales[i].id === id) {
      var originalCommissionUs = _stSaleReversalCommission(sales[i]);
...
function _stUpdateSaleStatus(id, newStatus) {
  ...
  if (idx < 0) return;
  var originalCommissionUs = _stSaleReversalCommission(sales[idx]);
    ...
    if (sales[si].id === dealId) {
      ...
      if (rowStatus === 'chargeback' || rowStatus === 'samecancel') {
        _stApplySaleReversal(
          sales[si],
          rowStatus,
          _stSaleReversalCommission(sales[si])
        );
      }
      break;
    }
...
function _stBuildTabHeroHtml(opts) {
  ...
  html +=
    ...
    '</div>';
  if (period) {
    html += '<div class="st-tab-hero-period">' + _stEscape(period) + '</div>';
  }
  html += '</div>';
  if (support) {
    html +=
...
// Commission actually reversed: core + add-ons, never the weekly
// $20 enrollment bonus. Older expectedDealTotal values still
// include that bonus; planCommission + totalAddonCommission do not.
function _stSaleReversalCommission(sale) {
  if (!sale) return 0;
  if (sale.type === 'deal') {
    var plan = Math.abs(Number(sale.planCommission) || 0);
    var addons = Math.abs(Number(sale.totalAddonCommission) || 0);
    if (plan > 0 || addons > 0) return _stMoney2(plan + addons);
    var expected = Math.abs(Number(sale.expectedDealTotal) || 0);
    var bonus = Math.abs(Number(sale.enrollmentBonus) || 0);
    if (expected) return _stMoney2(Math.max(0, expected - bonus));
    return 0;
  }
  if (typeof sale.addonCommission === 'number') {
    return Math.abs(Number(sale.addonCommission) || 0);
...
function _stReversalPaycheckWeekIsVerified(sale) {
  if (!sale) return false;
  if (_stPaycheckWeekIsVerified(sale.ts)) return true;
  var rev =
    sale.reversal && typeof sale.reversal === 'object' ? sale.reversal : null;
  if (!rev) return false;
  if (
    Number(rev.originalPaycheckWeek) &&
    _stPaycheckWeekIsVerified(rev.originalPaycheckWeek)
  ) {
    return true;
  }
  if (
    Number(rev.deductionPaycheckWeek) &&
    _stPaycheckWeekIsVerified(rev.deductionPaycheckWeek)
  ) {
    return true;
  }
  return false;
...
function _stRefreshSaleReversalAmount(sale) {
  if (!sale || !sale.reversal || typeof sale.reversal !== 'object') return;
  var amt = _stSaleReversalCommission(sale);
  sale.reversal.amountLost = amt;
  sale.reversal.originalCommission = amt;
}

// Reconcile compares itemized pay-sheet lines, so a core row must
// use only that plan's commission. Chargeback / CBC amounts use
// _stSaleReversalCommission (core + add-ons, no enrollment bonus).
...
function _stPaycheckFinishNumbers(pb) {
  ...
  if (pb.salary == null) pb.salary = _ST_PAYCHECK_SALARY_DEFAULT;
  ...
  if (pb.netPay != null && pb.salary != null) {
...
function _stApplySaleReversal(sale, newStatus, originalCommission) {
  ...
  var orig =
    typeof originalCommission === 'number' && isFinite(originalCommission)
      ...
      : _stSaleReversalCommission(sale);
  sale.reversal = {
    type: newStatus,
    eventTs: Date.now(),
    amountLost: orig,
...
function _stBackfillSaleReversal(sale) {
  ...
  var orig = _stSaleReversalCommission(sale);
  sale.reversal = {
    type: st,
...
function _stReconSheetTypeToLocal(t) {
  var x = String(t || '')
    .toLowerCase()
    .replace(/[^a-z]/g, '');
  if (x === 'core' || x === 'deal' || x === 'plan') return 'deal';
  if (
    x === 'ancillary' ||
    x === 'addon' ||
    x === 'addons' ||
...
function _stParsePaySheet(text) {
  ...
  for (i = 0; i < lines.length; i++) {
    ...
    if (
      low.indexOf('member id') !== -1 &&
      low.indexOf('product') !== -1 &&
      !typeLocal
    ) {
      continue;
    }
...
function _stReconWeekSales(sales, view) {
  view = view || _stRangeInfo();
  var weekStart = Number(view.start) || _stStartOfWeek(new Date()).getTime();
  var weekEnd =
    Number(view.endExclusive) || weekStart + 7 * 24 * 60 * 60 * 1000;
...
function _stReconSaleAmount(sale) {
  if (!sale) return 0;
  var n = Number(sale.amount);
  if (!isFinite(n)) n = 0;
  return Math.abs(n);
}
...
function _stMatchPaySheetRows(payRows, weekSales) {
  ...
  for (i = 0; i < payRows.length; i++) {
    var row = payRows[i];
    var rowMid = _stReconNormMemberId(row.memberId);
    var candidates = [];
    for (j = 0; j < weekSales.length; j++) {
      var s = weekSales[j];
      if (!s || usedLocal[s.id]) continue;
    ...
    if (exact) {
      matched += 1;
      usedLocal[exact.id] = true;
      continue;
    }

    if (candidates.length) {
      var localHit = candidates[0];
      usedLocal[localHit.id] = true;
      mislabeled.push({
        kind: 'mislabeled',
        pay: row,
        sale: localHit,
        customer: localHit.customer || row.customer || '',
        productName: localHit.plan || row.productName || '',
        dateTs: Number(localHit.ts) || 0,
        amount: _stReconSaleAmount(localHit)
      });
      continue;
    }

    // mid present on any week sale (already used) still counts as mislabeled
    var anyLocal = null;
    for (j = 0; j < weekSales.length; j++) {
      if (_stReconNormMemberId(weekSales[j].memberId) === rowMid) {
        anyLocal = weekSales[j];
        break;
      }
    }
    if (anyLocal) {
      mislabeled.push({
        ...
        pay: row,
        sale: anyLocal,
        customer: anyLocal.customer || row.customer || '',
        productName: row.productName || anyLocal.plan || '',
        dateTs: Number(anyLocal.ts) || 0,
        amount: _stReconSaleAmount(anyLocal)
      });
      continue;
    }
  ...
  for (j = 0; j < weekSales.length; j++) {
    var loc = weekSales[j];
    if (!loc || usedLocal[loc.id]) continue;
    var locMid = _stReconNormMemberId(loc.memberId);
    if (!locMid) continue;
    ...
    notOnSheet.push({
      ...
      productName: loc.plan || '',
      dateTs: Number(loc.ts) || 0,
      amount: _stReconSaleAmount(loc)
    });
  }

  var gap = 0;
  for (i = 0; i < missing.length; i++) gap += Number(missing[i].amount) || 0;
...
function _stOpenPaySheetModal() {
  _stClosePaySheetModal();
  var modal = document.createElement('div');
  modal.id = 'st-paysheet-modal';
  ...
  modal.innerHTML = html;
  modal.addEventListener('click', function (ev) {
    if (ev.target === modal) _stClosePaySheetModal();
  });
  document.body.appendChild(modal);
...
function _stClosePaySheetModal() {
  var modal = document.getElementById('st-paysheet-modal');
  if (modal && modal.parentNode) modal.parentNode.removeChild(modal);
}

function _stPaySheetModalIsOpen() {
...
function _stReconFormatDate(ts) {
  if (!ts) return '--';
  try {
    return _stWeekCardDateLabel(ts);
  } catch (_e) {
    var d = new Date(ts);
...
function _stReconcileChargebackCandidates(query) {
  ...
  for (i = 0; i < sales.length; i++) {
    var s = sales[i];
    if (!s) continue;
    var name = String(s.customer || '').toLowerCase();
    var mid = String(s.memberId || '');
    if (name.indexOf(q) === -1 && mid.indexOf(q) === -1) continue;
    hits.push(s);
...
function _stOnReconcileCbSearch(val) {
  _stCbSearchQuery = String(val || '');
  _stCbSelectedId = '';
  var box = document.getElementById('st-recon-cb-results');
  if (box) box.innerHTML = _stBuildReconcileCbResultsHtml();
}

function _stSelectReconcileCbSale(id) {
  _stCbSelectedId = String(id || '');
  var box = document.getElementById('st-recon-cb-results');
  if (box) box.innerHTML = _stBuildReconcileCbResultsHtml();
}

function _stBuildReconcileCbResultsHtml() {
  ...
  for (i = 0; i < hits.length; i++) {
    ...
    var st = _stNormalizeStatus(s);
    var pillMini = 'pending';
    if (st === 'chargeback') pillMini = 'chargeback';
    else if (st === 'cleared') pillMini = 'cleared';
    html +=
      ...
      _stEscape(s.plan || '') +
      (s.memberId ? ' · ID ' + _stEscape(String(s.memberId)) : '') +
      ' · ' +
      _stEscape(_stReconFormatDate(s.ts)) +
      '</div></div>' +
      '<span class="st-pill-mini st-pill-' +
      pillMini +
      '">' +
      _stEscape(_stStatusText(s)) +
      '</span></button>';
...
function _stReconcileMarkChargeback() {
  var id = String(_stCbSelectedId || '');
  if (!id) {
    _stFlash('Select a sale first.', 'warn');
    return;
  }
  _stUpdateSaleStatus(id, 'chargeback');
  _stCbSelectedId = '';
  _stCbSearchQuery = '';
  _stFlash('Sale marked as chargeback.', 'ok');
  _stRender();
}
    ...
    if (Number(prow.amount) < 0) {
      ...
      rows.push(
        _stMakeReconTableRow({
          ...
          dateTs:
            Number(
              (cbSale && cbSale.ts) || (exact && exact.ts) || prow.dateTs
            ) || 0,
          product:
            (cbSale && cbSale.plan) ||
            (exact && exact.plan) ||
...
function _stBuildReconcileChipsHtml(state) {
  ...
  var primary = { id: 'needs', label: 'Needs attention', n: c.needs };
  var secondary = [
    ...
    { id: 'samecancel', label: 'Cancels', n: c.samecancel }
  ];
  ...
  var html =
    '<div id="st-recon-chips" class="st-recon-v2-chips st-tab-filter-row" role="tablist">';
  html += chipHtml(primary);
  html += '<span class="st-filter-sep" aria-hidden="true">·</span>';
  var i;
  for (i = 0; i < secondary.length; i++) {
    html += chipHtml(secondary[i]);
  }
  html += '</div>';
  return html;
}
...
function _stBuildReconcileHistoryPane(sales) {
  ...
  var histHeadline =
    saved.length + ' saved reconciliation' + (saved.length === 1 ? '' : 's');
  var html =
  ...
  if (!saved.length) {
    ...
    for (i = 0; i < saved.length; i++) {
      ...
      if (pb.totalEarned != null) {
        figure = _stFmtMoney(pb.totalEarned);
      } else if (pb.netPay != null) {
        figure = _stFmtMoney(pb.netPay);
      }
      html +=
        ...
        '" data-week-label="' +
        _stEscape(weekLabel) +
        '" title="Delete" aria-label="Delete saved pay sheet">Delete</button>' +
        '</div></div></article>';
    }
    html += '</div>';
  }
...
function _stCbcEventFromSale(sale) {
  ...
  var rev =
    sale.reversal && typeof sale.reversal === 'object' ? sale.reversal : null;
  var origComm = _stSaleReversalCommission(sale);
  var lost = origComm;
...
function _stBuildCbcSummaryHtml(sum) {
  ...
  var support =
    'Lost to ' + sum.cbN + ' ' + cbWord + ' and ' + sum.swN + ' ' + swWord;
...
function _stBuildCbcTimeChipsHtml() {
  ...
  var i;
  for (i = 0; i < timeChips.length; i++) {
    html +=
      '<button type="button" class="st-cbc-chip' +
      (_stCbcTimeFilter === timeChips[i].id ? ' is-active' : '') +
      '" data-st-cbc-action="time" data-filter="' +
      timeChips[i].id +
      '">' +
      _stEscape(timeChips[i].label) +
      '</button>';
  }
  html += '<span class="st-filter-sep" aria-hidden="true">·</span>';
  for (i = 0; i < typeChips.length; i++) {
    html +=
      '<button type="button" class="st-cbc-chip' +
      (_stCbcTypeFilter === typeChips[i].id ? ' is-active' : '') +
      '" data-st-cbc-action="type" data-filter="' +
      typeChips[i].id +
      '">' +
      _stEscape(typeChips[i].label) +
      '</button>';
  }
  html += '</div>';
  return html;
...
function _stBuildPcTableRowHtml(rec) {
  ...
  var lostN = cbN + swN;
  var hasLost = pb.chargebacks != null || pb.sameWeekCancels != null;
  ...
  return (
    ...
    '</div>' +
    (actions
      ? '<div class="st-record-card-actions">' + actions + '</div>'
      : '') +
    '</div></article>'
  );
}
...
function _stBuildMarkReversalHitsHtml() {
  ...
  for (i = 0; i < hits.length; i++) {
    ...
    html +=
      ...
      _stEscape(_stFmtMoney(_stSaleReversalCommission(s))) +
      '</span></button>';
  }
  return html;
}
